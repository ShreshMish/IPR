...
unique_geographies <- readRDS("6 - Financial geog/Interim/TR_geographies_full_list.rds")

region_iso_code_data <- read_excel("6 - Financial geog/Input/ISO code and regional gdp.xlsx", 
                                   sheet = "W3. Region ISO code table", range = "$A$8:$IC$212")

gdp_iso_code_data <- read_excel("6 - Financial geog/Input/ISO code and regional gdp.xlsx",
                                sheet = "W4. GDP ISO code table", range = "$A$7:$C$211")

save_dated(gdp_iso_code_data, "GDP_ISO_code_data", folder = "Interim", csv = "YES")

#--------------------------------------------------------------------------------------------------

##### SECTION 2 - Clean datasets in preparation for region-ISO code matching

region_iso_code_data %<>%
...
region_iso_code_long_data <- region_iso_code_data %>%
  gather(key = "Region", value = "ISO_code_included", -(ISO_code:Country))

unique_mapped_regions <- unique(region_iso_code_long_data$Region)

unique_geographies %<>%
  filter(!(region %in% unique_mapped_regions))

save_dated(unique_geographies, "TR_unmapped_geographies", folder = "Interim", csv = "YES")

#--------------------------------------------------------------------------------------------------

##### SECTION 3 - Create new variabels for unmapped regions - this list was identified using the 
#####             TR geographies raw files output from Financial geog data cleaning.R script
...
region_iso_code_data %<>%
  mutate(`/Africa_Region/Western_Asia_Region` = Africa + Western_Asia, #Filled in example - note ticks applied to variable name
         `/Belgium_Country` = Belgium, #Filled in example - note ticks applied to variable name
         Abroad = World,
...
save_dated(region_iso_code_data, "TR_mapped_geographies_wide", folder = "Interim", csv = "YES")

region_iso_code_data %<>%
  gather(key = "Region", value = "Inclusion", -(ISO_code:Country)) %>%
  select(Region, everything())

save_dated(region_iso_code_data, "TR_mapped_geographies", folder = "Interim", csv = "YES")