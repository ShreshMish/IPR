...
save_dated <- function(data, name, folder, dated = "YES", csv = "NO") {
  main_path <- paste0("8 - Market parameters/", folder)
  dated_path <- paste0("8 - Market parameters/", folder, "/Dated/")
...
market_data <- readRDS("7 - Financial prod/Output/Final_markets_list.rds")

# Read in market parameterisation results
market_parameter_data <- read_excel("8 - Market parameters/Input/Market parameterisation_final.xlsx",
                                 sheet = "W1. Market parameterisation fin", range = "$G$9:$L$165")

# Read in MAC curve data
mac_curve_data <- readRDS("3 - MAC curves/Output/MAC_curve_cleaned_2016USD_data.rds")

# Read in carbon price data
carbon_price_data <- readRDS("0 - Scenarios/Output/Model_co2_prices_2016USD.rds")
