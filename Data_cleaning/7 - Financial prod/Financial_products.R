...
save_dated <- function(data, name, folder, dated = "YES", csv = "NO") {
  main_path <- paste0("7 - Financial prod/", folder)
  dated_path <- paste0("7 - Financial prod/", folder, "/Dated/")
...
product_market_allocation_1 <- read_excel("7 - Financial prod/Input/Market product classification_EM.xlsx",
...
product_market_allocation_2 <- read_excel("7 - Financial prod/Input/Company product classification_AT.xlsx",
                                          sheet = "180808 Company product classifi", range = "$A$1:$P$1946")

# Rystad analysed oil and gas companies
oil_and_gas_companies <- readRDS("4 - Fossil fuels/Oil and gas/Output/DD_analysis_companies_list.rds") %>%
  # Filter out companies not included in Justine's analysis
  filter(!(company %in% c("CABOT OIL & GAS 'A'", "GENTING")))

# CO2 emissions data
co2_emissions_data <- readRDS("2 - CO2 emissions/Output/Trucost_cleaned_emissions_data.rds")
...
save_dated(financial_data_for_aaron, "Company_product_classification", folder = "Interim", csv = "YES")
...
save_dated(markets_for_aaron, "Markets_for_classification", folder = "Interim", csv = "YES")
...
save_dated(final_markets, "Final_markets_list", folder = "Output", csv = "YES")

# Save product exposure data for 2016
product_exposure_results_2016 <- product_exposure_results %>%
  filter(year == 2016)

save_dated(product_exposure_results_2016, "Market_exposure_results_2016", folder = "Output", csv = "YES")

# Save product exposure data for model year (2017)
product_exposure_results %<>%
  filter(year == 2017)

save_dated(product_exposure_results, "Market_exposure_results", folder = "Output", csv = "YES")
