...
save_dated <- function(data, name = " ", folder, dated = "YES", csv = "NO") {
  main_path <- paste0("2 - CO2 emissions/", folder)
  dated_path <- paste0("2 - CO2 emissions/", folder, "/Dated/")
...
emissions_raw_data <- read_excel("2 - CO2 emissions/Input/Carbon Emissions Screen_v1.0.xlsx", sheet = "Carbon Emissions",
                                 range = "$A$6:$R$27853")

save_dated(emissions_raw_data, "Emissions_raw_data", folder = "Interim")
...
save_dated(missing_companies, "Trucost_missing_companies", folder = "Interim", csv = "YES")

# Read in missing companies analysis and merge with Trucost data based on new set of ISIN codes

missing_companies_analysis <- read_excel("2 - CO2 emissions/Input/TR missing companies analysis.xlsx",
...
save_dated(company_emissions_data, "Trucost_emissions_data_full", folder = "Interim", csv = "YES")

company_emissions_data %<>%
  filter(year == 2017)

save_dated(company_emissions_data, "Trucost_cleaned_emissions_data", folder = "Output", csv = "YES")