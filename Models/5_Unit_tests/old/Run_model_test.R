...
source("3_Cost_and_competition/Dummy_model.R")
...
# ### 2-d checking, for info
#
# parameter_dataCPT <- tibble(discount_rate = 0.0575, #Changing discount rate from 0.0775 to 0.0575
#                             competition = 0.5,
#                             product_differentiation = "change",  #JC: as long as this isn't B.0, it will iterate correctly 
#                             num_firms = "ON", #Changed from perfect competition (PC = 10^9 firms) to "ON" (endogenous number of firms based on H index)
#                             firm_closure = "ON",
#                             industry_cost_pass_through = 0.7, # this can be changed and will be iterated over in final results.
#                             elasticity = 1, #JC: as long as this isn't B.0, it will iterate correctly
#                             stranding = "OFF",
#                             green_upside = "OFF",
#                             power_model = "OFF",
...
# }
# 
# artificial_wrapCPT <- function(x) {
#   
#   temp <- parameter_dataCPT %>%
#     # mutate(elasticity = x) # replaces elasticity to mapped value
#     mutate(product_differentiation = x) 
#   print(paste0("prod. differentiation is", " ", x)) # helps keep track of progress
#   
#   other_model(artificial_data, temp, "", variables = quo(c("market_cap_2017", starts_with("profit_post_closure_pre_tax"),
#                                                            starts_with("profit_post_closure_post_tax"),
#                                                            starts_with("profit_npv_post_closure_post_tax"),
...
# }
# 
# artificial_results <- map(1:2, artificial_wrap) %>% # can replace domain elas with 1:3 to save time
#   bind_rows() 
# 
# artificial_results2 <- map(c(0.005, 0.5), artificial_wrapCPT) %>% # JC: will replace domain 1:3 with elas when ready to run all 40 values
#   bind_rows() 
#  
# 
# summarise_artificial <- function(...) {
...
# }
# 
# artificial_res2 <- summarise_artificial(elasticity)
#
# 
# # 2d charts, for info   
# plot(x = artificial_res2$elasticity, 
#      y = artificial_res2$index, 
#      type = "b", 
#      xlab = "elasticity", 
#      ylab = "Profit impact under balanced 2DS relative to Paris NDCs")
# 
# 
# plot(x = artificial_res$product_differentiation, 
#      y = artificial_res$index, 
#      type = "b", 
#      xlab = "product_differentiation", 
#      ylab = "Profit impact under balanced 2DS relative to Paris NDCs")
