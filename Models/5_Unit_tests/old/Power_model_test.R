  ...
  for(i in model_years) {
    ...
    rr_panel_model_run %<>%
      ...
      mutate(!!delta_unit_cost_t_new := co2_intensity_scope_1 * ((!!carbon_cost_t_new) - (!!carbon_cost_t_old)) 
             ...
             !!industry_cost_pass_through_t_new := rr_parameter_data$industry_cost_pass_through,
...
power_model <- function(data, parameter_data, value_chain_element, variables) {
  ...
  return(list(pwh_carbon_cost_results))
}
