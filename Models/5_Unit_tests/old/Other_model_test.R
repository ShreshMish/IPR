...
run_rr_model <- function(rr_data, rr_carbon_cost_pwh_data = NULL, rr_parameter_data, rr_value_chain_element) {
  ...
  rr_panel_model_run <- rr_data # first call this is the case, so runs power sector model
  ...
  for(i in model_years) {
    ...
    rr_panel_model_run %<>%
      ...
      mutate(!!carbon_cost := if(rr_parameter_data$abatement_opportunities == "ON") (!!carbon_cost) else (!!carbon_price),
             !!carbon_cost_pwh := if(rr_parameter_data$power_model == "ON") (!!carbon_cost_pwh) else (!!carbon_price))
  }
  ...
  for(i in model_years) {
    ...
    rr_panel_model_run %<>%
      ...
      mutate(!!delta_unit_cost_t_new := co2_intensity_scope_1 * ((!!carbon_cost_t_new) - (!!carbon_cost_t_old)) 
             ...
             # JC: could add a line here: industry_cost_pass_through_t_new = parameter_data...
...
other_model <- function(data, parameter_data, value_chain_element, variables) {
  ...
  run_results <- run_rr_model(rr_data = all_other_panel_model_run,rr_parameter_data = parameter_data, 
                              rr_value_chain_element = value_chain_element)
  ...
  subsidiary_results <- run_results2 %>%
    select(elasticity, product_differentiation, scenario, company_id, company, market, region, market_cap_2017, market_cap_model, !!(variables)) %>%
